...
export default new ModuleBuilder('blend_first_image_scene', (module: Module) => {
  module.scene?.enter(async (ctx) => {
    await ctx.reply(config.first_photo, {
      parse_mode: 'Markdown'
    })
  })
  module.scene?.on('photo', async (ctx) => {
    ...
    await ctx.scene.enter('blend_second_image_scene')
  })
  return module;
})
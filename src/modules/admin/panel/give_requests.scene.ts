...
export default new ModuleBuilder('give_requests_scene', (module: Module) => {
  ...
  module.scene?.on('text', async (ctx) => {
    ...
    await ctx.reply(`Пользователю [${user?.name}](tg://user?id=${admin?.userId}) (ID: \`${admin?.userId}\`) выданы запросы в количестве ${num}`, {
      parse_mode: 'Markdown'
    })
